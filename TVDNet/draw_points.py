...
def draw_landmarks_regress_test(pts0,ori_image_heatmap, ori_image_regress, ori_image_points):
    for i, pt in enumerate(pts0):
        # color = np.random.rand(3)
        color = colors[i]
        # ori_image_heatmap = ori_image_regress.
        # print(i+1, color)
        color_255 = (255 * color[0], 255 * color[1], 255 * color[2])
        color_255 = (0,0,255)
        cv2.circle(ori_image_heatmap, (int(pt[0]), int(pt[1])), 6, color_255, -1, 1)
        ...
        cv2.circle(ori_image_points, (int(pt[2]), int(pt[3])), 4, color_255, -1, 1)
        cv2.circle(ori_image_points, (int(pt[4]), int(pt[5])), 4, color_255, -1, 1)
        cv2.circle(ori_image_points, (int(pt[6]), int(pt[7])), 4, color_255, -1, 1)
        cv2.circle(ori_image_points, (int(pt[8]), int(pt[9])), 4, color_255, -1, 1)
    return ori_image_heatmap, ori_image_regress, ori_image_points
