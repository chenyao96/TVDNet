...
def generate_ground_truth(image,
    ...
    # mid_point = np.zeros((17,2),dtype=np.float32)
    points4 = np.zeros((17, 2*4), dtype=np.float32)

    next_cen = np.zeros((17, 2), dtype=np.float32)
    next_vector = np.zeros((17, 2), dtype=np.float32)
    ...
    for k in range(17):
        ...
        if k == 16:
            ...
            pts_ = pts_2[4 * (k + 1):4 * (k + 1) + 4, :]
            cen_x_, cen_y_ = np.mean(pts_, axis=0)
            ct_ = np.asarray([cen_x_, cen_y_], dtype=np.float32)
            pass

        # ct_int_ = ct_.astype(np.int32)
        ...
        next_cen[k] = ct_
        # 中心点的向量偏差
        next_vector[k] = ct_ - ct

        # mid_point[k] = ct_-ct  #中心点的向量约束
        reg_mask[k] = 1
        for i in range(4):
            wh[k,2*i:2*i+2] = ct-pts[i, :]
            points4[k, 2 * i:2 * i + 2] = pts[i, :]
...
# def generate_ground_truth2(image,
#                           pts_2,
#                           image_h,
#                           image_w,
#                           img_id):
